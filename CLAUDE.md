...
- **`docs/runbooks/comfyui.md`** is the operational runbook for the render container: model directory map, the two memory ceilings and the exit-137 OOM diagnosis path, the pinned Torch/CUDA base image and its runtime patch, custom nodes with hardcoded Docker hostnames, and nodes that are deprecated/regime-incompatible in the LTX 2.3 distilled graphs. Read it before touching a render failure. Infra changes to that container go through `homelab-ai/infra/scripts/apply-infra-change.sh` and the `infra-change` skill — never edit-and-restart by hand.
- **`docs/video-acceptance.md`** (generated from `specs/video-spec.json` by `scripts/gen_video_acceptance_doc.py`) is the hard, machine-checkable acceptance bar for generated video: pt-BR diction, no Spanish cue in the prompt, silence/speech-ratio/duration limits, photorealistic not cartoon. `scripts/lint-prompt.sh` gates prompts before render; `scripts/grade-video.sh` scores a render (every rule a computed number); `scripts/video-accept-loop.sh` is the lint→render→grade→mutate loop (cap 5, never switches approach on its own).
